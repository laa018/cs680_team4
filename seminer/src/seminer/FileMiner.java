...
public class FileMiner {
	...
	private final static String subquery = "SELECT DISTINCT files.id as file_id, file_links.file_path as file_path, actions.branch_id as branch_id FROM files LEFT OUTER JOIN file_links ON file_links.file_id = files.id LEFT OUTER JOIN actions ON actions.file_id = files.id";
	private final static String tagquery = "SELECT DISTINCT tags.name FROM tags LEFT OUTER JOIN tag_revisions ON tag_revisions.tag_id = tags.id LEFT OUTER JOIN file_links ON file_links.commit_id = tag_revisions.commit_id LEFT OUTER JOIN actions ON actions.file_id = file_links.file_id";
	...
	public void mine(int repositoryId, String projectName) {
		...
		try {
			Repositories repository = (Repositories)cvsanalySession.createQuery("FROM Repositories WHERE id = " + repositoryId).uniqueResult();
			
			List<FileLinks> result = cvsanalySession.createSQLQuery("SELECT file_links.* from file_links LEFT OUTER JOIN files ON file_links.file_id = files.id LEFT OUTER JOIN file_types ON file_types.file_id = file_links.file_id WHERE files.repository_id = " + repositoryId + " AND file_types.type IS NOT NULL").addEntity("file_links", FileLinks.class).list();
			...
			for(int i = 0; i < pathSets.size(); i++) {
				...
				for (String string : paths) {
					...
					List<Object[]> results = cvsanalySession.createSQLQuery("select {file_types.*}, {branches.*} from (" + subquery + ") s LEFT OUTER JOIN file_types ON file_types.file_id = s.file_id LEFT OUTER JOIN branches ON branches.id = s.branch_id WHERE s.file_path = '" + string + "'").addEntity("file_types", FileTypes.class).addEntity("branches", Branches.class).list();
					for (Object[] objects : results) {
						...
						if(usedBranches.contains(branch.getName())) {
							if(verbose) {
								System.out.println("Skipping " + type.getFileId() + " repeat path and branch");
							}
							continue;
						} else {
						...
						if(verbose) {
							System.out.println("Processing " + type.getFileId() + " " + string + " on branch " + branch.getName());
						}
						
						String release_number = (String)cvsanalySession.createSQLQuery(tagquery + " where file_links.file_id = " + type.getFileId() + " AND file_links.file_path = '" + string + "'" + " AND actions.branch_id = " + branch.getId()).uniqueResult();
						...
						List<Metrics> metricsList = cvsanalySession.createSQLQuery("SELECT s.*, max(date) FROM (SELECT * FROM metrics WHERE file_id = '" + type.getFileId() + "') s LEFT OUTER JOIN scmlog ON scmlog.id = s.commit_id").addEntity("s", Metrics.class).list();
						if(metricsList.size() > 0 && metricsList.get(0) != null) {
							...
							effortMetricsFile.setHalstead_level((metrics.getHalsteadLevel() != null) ? Integer.valueOf(metrics.getHalsteadLevel().intValue()) : null);
						...
						if(write) {
							...
							ActionMiner.update(effortMetricsSession, projectName, type.getFileId(), effortMetricsFile);
						}
					}
				}
			}
