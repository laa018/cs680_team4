...
/**
 * Home object for domain model class People.
 * @see bicho.People
 * @author Hibernate Tools
 */
public class PeopleHome {

	private static final Log log = LogFactory.getLog(PeopleHome.class);
	...
	public void persist(People transientInstance) {
		log.debug("persisting People instance");
		try {
	...
	public People merge(People detachedInstance) {
		...
		try {
			People result = (People) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public People findById(java.lang.Integer id) {
		...
		try {
			People instance = (People) sessionFactory.getCurrentSession().get(
					"bicho.People", id);
			if (instance == null) {
